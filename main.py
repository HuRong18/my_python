...
def dataset(batch_size):
    ...
    for i in range(batch_size):
        ...
        for j in range(3):
            ...
            # a = A.T
            # x = (A + a)/2
            # f = np.linalg.eigvals(x).reshape(const.dimension, 1)
            # x = np.random.rand(const.dimension, const.dimension)
            # f = 1+x+x**2
...
def train():
    dnn = DNN.DNN([1 / 4, 2 / 4, 3 / 4], device)
    loss = 1
    i = 1
    for epoch in range(1000):
        torch.set_default_dtype(torch.float64)
        x, feature = dataset(32)
        input = torch.from_numpy(x).to(torch.float64).to(device)
        expect: torch.Tensor = torch.from_numpy(feature).to(torch.float64).to(device)
...
def predict(filename):
    test_net = DNN.DNN(const.alpha_list, device)
